...
import BgImages from '../../public/prestasibg.jpg'
import HTMLReactParser, { domToReact } from "html-react-parser"
import moment from "moment"
import Link from "next/link"

export default function RekamJejakList({detailPrestasi}){
  const CardPrestasi = ({data}) => {
    return (
      <div>
        <div className="sm:flex p-5 sm:p-0">
          ...
          <Link href={'/prestasi/' + data.slug}>
            <h1 className="text-2xl mb-2 cursor-pointer hover:text-yellow-500">{data.title}</h1>
            </Link>            <h1 className="mb-1 text-gray-600">{moment(data.date).format('LL')}</h1>
  ...
  return <Layout>
    <div className='flex items-center justify-center w-full relative sm:py-10'>
      <div className="w-full h-half-screen relative">
        ...
        <div className="absolute bottom-0 left-0 right-0 top-0 flex justify-center items-center z-100">
          <h1 className=" text-5xl text-white drop-shadow-2xl">PRESTASI</h1>
        </div>
      </div>
    </div>
    <div className="flex justify-center items-center">
      <div className="sm:w-3/4">
        {
          detailPrestasi.map((e,index) => <CardPrestasi key={index} data={e} />)
        }
      </div>
    </div>
  </Layout>
...
export async function getStaticProps() {
  const getPrestasiData = await fetch('https://kuapi.rucciartcycles.com/wp-json/wp/v2/posts?categories=8')
  const dataPrestasi = await getPrestasiData.json()
  const detailPrestasi = await getDataDetails(dataPrestasi)
  return {
      props: {
        detailPrestasi
      }
