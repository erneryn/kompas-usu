    ...
    return (
      <div>
        <div className="sm:flex p-5 sm:p-0">
          ...
          <Link href={'/news-and-article/' + data.slug}>
  ...
  return <Layout>
    <div className='flex items-center justify-center w-full relative sm:py-10'>
      <div className="w-full h-half-screen relative">
        <Image src={BgImages} layout="fill" objectFit="cover" objectPosition="bottom"  />
        <div className="absolute bottom-0 left-0 right-0 top-0 flex justify-center items-center z-100">
          <h1 className=" text-3xl text-white drop-shadow-2xl">NEWS AND ARTICLE</h1>
        </div>
      </div>
    </div>
...
export async function getStaticProps() {
  const getRekamJeajakData = await fetch('https://kuapi.rucciartcycles.com/wp-json/wp/v2/posts?categories=7')
